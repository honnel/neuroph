...
public class ScoreCalculatorSuite {
	...
	public static void main(String[] args) throws IOException {
		...
		DataSet cernDataSet = DataSet.createFromFile("/Users/dominic/Documents/Uni/Semester07/Multikernprogrammierung/mc_prac/neuroph2.7/data/cern/15000rows.txt", CERN_FULL_INPUTCOUNT, 1, ",");
	...
	private void testClonebasedAndRevisedAndNormalMLP(DataSet dataSet, int hiddenNeuronCount, int maxIterations, int numThreads, int syncFrequency, int runs,
			double trainingSetRatio) {

		System.out.println("+-+-+-+-+ Test Configuration +-+-+-+-+");
		System.out.println("Clonebased, Clonebased Revised, Seq MLP: " + hiddenNeuronCount);
		System.out.println("Hidden Neurons: " + hiddenNeuronCount);
		System.out.println("Maximum Learning Iterations: " + maxIterations);
		System.out.println("#Threads: " + numThreads);
		System.out.println("For Unrevised: Interpolate clones after every <#> rows: " + syncFrequency);
		...
		int i = 0;
		learners[0] = new ClonebasedConcurrentLearnerRevised(numThreads, maxIterations, NeuralNetInterpolatorType.ArithmeticMean, neuralNet,
				"Clonebased-Revised ArithmeticMean");
	...
	private void testClonebasedAndNormalMLP(DataSet dataSet, int hiddenNeuronCount, int maxIterations, int numThreads, int syncFrequency, int runs,
		...
		System.out.println("Maximum Learning Iterations: " + maxIterations);
		System.out.println("#Threads: " + numThreads);
		System.out.println("Interpolate clones after every <#> rows: " + syncFrequency);
		...
		for (NeuralNetInterpolatorType type : types) {
			learners[i++] = new ClonebasedConcurrentLearner(numThreads, syncFrequency, type, neuralNet);
		}
	...
	private void testParallelBatchLearningRule(DataSet dataSet, int hiddenNeuronCount, int maxIterations, int numThreads, int runs, double trainingSetRatio) {
		System.out.println("+-+-+-+-+ Test Configuration +-+-+-+-+");
		System.out.println("Hidden Neurons: " + hiddenNeuronCount);
		System.out.println("Maximum Learning Iterations: " + maxIterations);
		System.out.println("#Threads: " + numThreads);
		...
		MultiLayerPerceptron parNeuralNet = new MultiLayerPerceptron(dataSet.getInputSize(), hiddenNeuronCount, 1);
		parNeuralNet.setLearningRule(new BatchParallelBackpropagation(numThreads));
		((LMS) parNeuralNet.getLearningRule()).setMaxIterations(maxIterations);
		NeuralNetworkWrapper parBatch = new NeuralNetworkWrapper(parNeuralNet, numThreads, "Parallel MLP BatchBackpropagation");

		MultiLayerPerceptron seqNeuralNet = new MultiLayerPerceptron(dataSet.getInputSize(), hiddenNeuronCount, 1);
		((LMS) seqNeuralNet.getLearningRule()).setBatchMode(true);
		((LMS) seqNeuralNet.getLearningRule()).setMaxIterations(maxIterations);
		NeuralNetworkWrapper seqBatch = new NeuralNetworkWrapper(seqNeuralNet, 1, "Sequential MLP BatchBackpropagation");

		Score[] scores = ScoreCalculator.trainAndCalculateOnPermutedSet(dataSet, trainingSetRatio, runs, parBatch, seqBatch);
	...
	private void trainOneElementSet() {
		...
		oneElementSet.addRow(new double[] { 0.5, 0.7, 0.3, 0.1 }, new double[] { 0.4, .2, 1 });
