...
public abstract class NeuralNetwork implements Serializable {
	 ...
	 */
	private transient javax.swing.event.EventListenerList listeners = new javax.swing.event.EventListenerList();
	...
	public void addLayer(Layer layer) {
		...
		this.layers = Arrays.copyOf(layers, layers.length + 1); // grow existing
																// layers array
																// to make space
																// for new layer
		this.layers[layers.length - 1] = layer; // add new layer at the end of
												// array
	}
	...
	public void addLayer(int index, Layer layer) {
		layer.setParentNetwork(this);
		this.layers = Arrays.copyOf(layers, layers.length + 1); // first grow
																// array to make
																// space for new
																// layer

		for (int i = layers.length - 1; i > index; i--) // shift all layers
														// right
		{
	...
	public void setInput(double... inputVector) throws VectorSizeMismatchException {
		...
		for (Neuron neuron : this.inputNeurons) {
			neuron.setInput(inputVector[i]); // set input to the coresponding
												// neuron
			i++;
		}
	...
	public double[] getOutput() {
		// double[] outputVector = new double[outputNeurons.length];// use
		// attribute to avoid creating to arrays and avoid GC work
