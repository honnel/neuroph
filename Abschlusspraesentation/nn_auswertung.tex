...
\usetikzlibrary{automata} 
  ...
  \begin{tikzpicture}[->,shorten >=1pt,auto, node distance=1cm,
    ...
    %   \foreach \x in {0,..., 2} {
    %     \ifthenelse{\x=1}{
    %       \node[main node] (2\x) at (8,5.2-1.2*\x) {$1-\pgfmathparse{\allrightresultstransfered[\x}\pgfmathresult=\pgfmathparse{\error[\x}\pgfmathresult$};
    %     }{
    %       \node[main node] (2\x) at (8,5.2-1.2*\x) {$0-\pgfmathparse{\allrightresultstransfered[\x}\pgfmathresult=\pgfmathparse{\error[\x}\pgfmathresult$};
    %     }
    %   }
